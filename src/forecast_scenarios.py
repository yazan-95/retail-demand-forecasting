    ...
    for i, ((store_id, product_id), group) in enumerate(sku_groups):
        ...
        try:
            ...
            def update_price_features(temp_row, new_price):
                ...
                # Core price
                # ----------------------------
                new_price = np.float32(new_price)
                ...
                if "price_avg_7" in temp_row.columns:
                    ...
                    temp_row.loc[idx, "price_vs_avg_7"] = np.float32(
                        new_price / avg
                    )
                ...
                if "price_avg_28" in temp_row.columns:
                    ...
                    temp_row.loc[idx, "price_vs_avg_28"] = np.float32(
                        new_price / avg
                    )
                ...
                if (
                    ...
                    std = max(
                        float(temp_row.loc[idx, "price_std_28"]),
                        1e-6
                    )

                    temp_row.loc[idx, "price_zscore_28"] = np.float32(
                        (new_price - avg) / std
                    )
                ...
                if "price_vs_avg_28" in temp_row.columns:
                    temp_row.loc[idx, "discount_depth"] = np.float32(
                        1.0 - temp_row.loc[idx, "price_vs_avg_28"]
                    )
                ...
                if (
                    ...
                    temp_row.loc[idx, "price_momentum"] = np.float32(
                ...
                if "price_squared" in temp_row.columns:
                    temp_row.loc[idx, "price_squared"] = np.float32(
                        new_price ** 2
                    )

                if "log_price" in temp_row.columns:
                    temp_row.loc[idx, "log_price"] = np.float32(
                        np.log1p(new_price)
                    )
                ...
                if (
                        ...
                        and
                        "discount_depth" in temp_row.columns
                        and
                        "promo_price_interaction" in temp_row.columns
                ):
                    temp_row.loc[idx, "promo_price_interaction"] = np.float32(
